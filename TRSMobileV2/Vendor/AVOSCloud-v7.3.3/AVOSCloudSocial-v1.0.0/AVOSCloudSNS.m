...
+(void)onSuccess:(AVOSCloudSNSType)type withToken:(NSString*)token andExpires:(NSString*)expires andUid:(NSString*)uid{
    ...
    [[self requestManager] GET:url parameters:params progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
    ...
    switch (type) {
        case AVOSCloudSNSSinaWeibo:
        {
            ...
            if (dict) {
                ...
                [[self requestManager] GET:@"https://api.weibo.com/oauth2/revokeoauth2" parameters:@{@"access_token":token} progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
                } failure:^(NSURLSessionDataTask *task, NSError *error) {
                }];
            }
        }
...
+ (void)getWeixinAccessTokenByCode:(NSString *)code block:(AVSNSResultBlock)block{
    ...
    [manager GET:@"https://api.weixin.qq.com/sns/oauth2/access_token" parameters:params progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
