...
-(void)getAccessToken:(NSString*)code{
    ...
    [[AVOSCloudSNS requestManager] POST:url parameters:param progress:nil success:^(NSURLSessionDataTask *task, id responseObject){
...
-(BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType{
    ...
    if ([url rangeOfString:avos_callback_prefix].length>0||[url rangeOfString:avos_callback_prefix2].length>0) {
        ...
        if(hasCode){
            //avos返回用户信息
            [[AVOSCloudSNS requestManager] GET:url parameters:nil progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
                NSDictionary *info=[NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingAllowFragments error:nil];
                info=info[@"user"];
                if (info) {
        ...
        if (code) {
            ...
            [[AVOSCloudSNS requestManager] GET:[NSString stringWithFormat:@"https://graph.qq.com/oauth2.0/me?access_token=%@",token] parameters:nil progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
