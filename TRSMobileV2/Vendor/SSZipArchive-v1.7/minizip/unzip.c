...
extern int ZEXPORT unzOpenCurrentFile3(unzFile file, int *method, int *level, int raw, const char *password)
{
    ...
    if ((password != NULL) && ((s->cur_file_info.flag & 1) != 0)) {
        ...
        if (s->cur_file_info.compression_method == AES_METHOD) {
            ...
            fcrypt_init((int)s->cur_file_info_internal.aes_encryption_mode, (unsigned char *)password, (unsigned int)strlen(password), saltvalue, passverify_password, &s->pfile_in_zip_read->aes_ctx);
