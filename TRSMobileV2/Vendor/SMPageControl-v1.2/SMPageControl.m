...
- (void)_renderPages:(CGContextRef)context rect:(CGRect)rect
{
	...
	for (NSInteger i = 0; i < _numberOfPages; i++) {
		...
		if (image) {
            ...
            if(!_isSamePointRadius) {
                CGFloat _w = (i == _displayedPage ? _measuredIndicatorWidth : _measuredIndicatorWidth * 0.75);
                CGFloat _h = (i == _displayedPage ? _measuredIndicatorHeight : _measuredIndicatorHeight * 0.75);
                CGFloat _yoffset = (self.center.y - _h/2.0);
                CGContextFillEllipseInRect(context, CGRectMake(xOffset, yOffset + _yoffset, _w, _h));
            }
            else {
