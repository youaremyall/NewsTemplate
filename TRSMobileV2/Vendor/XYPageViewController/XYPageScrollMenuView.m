...
@interface XYPageScrollMenuView ()
...
/// ScrollView
@property (nonatomic, strong)  XYPageScrollView *scrollView;
/// Add按钮
...
/// 配置信息
@property (nonatomic, strong) XYPageConfigration *configration;
/// 代理
@property (nonatomic, weak) id<XYPageScrollMenuViewDelegate> delegate;
/// 上次index
...
@implementation XYPageScrollMenuView
...
+ (instancetype)pagescrollMenuViewWithFrame:(CGRect)frame
    ...
    XYPageScrollMenuView *menuView = [[XYPageScrollMenuView alloc] initWithFrame:frame];
    menuView.titles = titles;
    menuView.delegate = delegate;
    menuView.configration = configration ?: [XYPageConfigration defaultConfig];
...
- (UIScrollView *)scrollView {
    if (!_scrollView) {
        _scrollView = [[XYPageScrollView alloc] init];
