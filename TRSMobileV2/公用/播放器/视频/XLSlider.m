...
- (void)drawLayer:(CALayer *)layer inContext:(CGContextRef)ctx {
    ...
    if(isnan(panDistance)) {panDistance = 0.01f;}
    if(isnan(self.middleValue)) {self.middleValue = 0.01f;}
...
- (instancetype)init {
    ...
    if (self = [super init]) {
        ...
        _lineLayer.delegate = (id<CALayerDelegate>)_delegate;
...
- (UIColor *)sliderColor {
    if (!_sliderColor) {
        return [UIColor colorWithRGB:UIColorThemeDefault];
    }
    return _sliderColor;
}
